{
  inputs =
    let
      version = "1.6.1";
      system = "x86_64-linux";
      devenv_root = "/home/esdevries/code/CrabLock";
      devenv_dotfile = ./.devenv;
      devenv_dotfile_string = ".devenv";
      container_name = null;
      devenv_tmpdir = "/run/user/1000";
      devenv_runtime = "/run/user/1000/devenv-a7e4394";
    ...
    // (
      if builtins.pathExists (devenv_dotfile + "/flake.json") then
        builtins.fromJSON (builtins.readFile (devenv_dotfile + "/flake.json"))
      else
        { }
    );
  ...
  outputs =
    ...
    let
      ...
      devenv =
        if builtins.pathExists (devenv_dotfile + "/devenv.json") then
          builtins.fromJSON (builtins.readFile (devenv_dotfile + "/devenv.json"))
        else
          { };
      getOverlays =
      ...
      importModule =
        ...
        if lib.hasPrefix "./" path then
          ...
          throw "devenv: ../ is not supported for imports"
        else
          let
      ...
      project = pkgs.lib.evalModules {
        ...
        modules =
          ...
          ++ [
            ...
            (
              if builtins.pathExists (devenv_dotfile + "/cli-options.nix") then
                import (devenv_dotfile + "/cli-options.nix")
              else
                { }
            )
          ];
